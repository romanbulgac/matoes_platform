...
export const GroupConsultationsList: FC<GroupConsultationsListProps> = ({
  ...
  // ============= Filtering Logic =============
  // Type guard для проверки статуса
  const isConsultationStatus = (status: FilterStatus): status is ConsultationStatus => {
    return status !== 'all';
  };

  const filteredConsultations = useMemo(() => {
    if (filterStatus === 'all') return consultations;
    // @ts-ignore - TypeScript не может правильно вывести тип после условной проверки
      ...
      <div className={className}>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <div className="flex-1 flex items-center gap-2">
            ...
            <Select value={filterStatus.toString()} onValueChange={(v) => {
              if (v === 'all') {
                setFilterStatus('all');
              } else {
                setFilterStatus(Number(v) as ConsultationStatus);
              }
            }}>
              ...
              <SelectContent>
                ...
                <SelectItem value={ConsultationStatus.Scheduled.toString()}>Programate ({getStatusCount(ConsultationStatus.Scheduled)})</SelectItem>
                <SelectItem value={ConsultationStatus.InProgress.toString()}>În desfășurare ({getStatusCount(ConsultationStatus.InProgress)})</SelectItem>
                <SelectItem value={ConsultationStatus.Completed.toString()}>Finalizate ({getStatusCount(ConsultationStatus.Completed)})</SelectItem>
                <SelectItem value={ConsultationStatus.Cancelled.toString()}>Anulate ({getStatusCount(ConsultationStatus.Cancelled)})</SelectItem>
              </SelectContent>
            </Select>
          </div>
