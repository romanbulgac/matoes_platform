// Сервис консультаций
import { ConsultationDto, ConsultationStatus, ConsultationType, CreateConsultationDto, MaterialDto, UserDto } from '../types/api';
...
function convertApiConsultationToDto(apiData: Record<string, unknown>): ConsultationDto {
  ...
  return {
    ...
    consultationType: apiData.consultationType as ConsultationType,
...
export class ConsultationService {
  ...
  static async updateAttendance(
    ...
    console.log('📝 Updating attendance:', { consultationId, studentId, attended });
  ...
  /**
   * Obține informații despre prezență pentru o consultație
   */
  static async getAttendance(consultationId: string): Promise<Array<{
    ...
    console.log('📋 Getting attendance for consultation:', consultationId);
  ...
  static async getStudentIndividualConsultations(studentId: string): Promise<ConsultationDto[]> {
    const consultations = await this.getStudentConsultations(studentId, 'all');
    return consultations.filter(c => c.consultationType === ConsultationType.Individual);
  }
  ...
  static async getStudentGroupConsultations(studentId: string): Promise<ConsultationDto[]> {
    const consultations = await this.getStudentConsultations(studentId, 'all');
    return consultations.filter(c => c.consultationType === ConsultationType.Group);
  }
